...
def plot_output(u, p, epoch, plotdir, suffix=''):
    fig = plt.figure()
    plt.bar(range(1,1+u.size()[0]), u.detach().cpu().squeeze(), color='k')
    plt.xlabel("output number")
    plt.ylabel("output")
    plt.gca().xaxis.set_major_locator(MaxNLocator(integer=True))
    fig.savefig(f"{plotdir}output_epoch{epoch}_X{p}{suffix}.png")
    #fig.savefig(plotdir+'output_epoch%d_X%d.png' % (epoch, p))
    plt.close(fig)
...
def output_timeplot(mz, probe_x, probe_y, fig_filename='', mz_name=''):
    fig = plt.figure()
    mz_data = mz[:,probe_x, probe_y]
    plt.plot(mz_data)
    plt.xlabel("time")
    plt.ylabel("mz")
    plt.ylim([-0.04, 0.04])
    ...
    plt.close(fig)
    if mz_name:
        df = pd.DataFrame({'mz': mz_data})
        df.to_excel(mz_name, index=True)
