...
class MMSolver(nn.Module):
    ...
    def __init__(self, geometry, dt: float, sources=[], probes=[]):
        ...
        self.fwd = False  # differentiates main fwd pass and checkpointing runs
    ...
    def forward(self, list_signal): 
        ...
        for sig_num in range(len(list_signal)):
            ...
            self.m_history = []
            self.fwd = True
            B_ext = self.geom()        # always call .forward() to get external field
            Msat = self.geom.Ms        # always access Msat from geometry
